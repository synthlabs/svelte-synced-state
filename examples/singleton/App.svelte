<script lang="ts">
	...
	import { SyncedState, singletonAddress } from 'svelte-synced-state';
	...
	const appState = new SyncedState<AppState>(singletonAddress('appstate'), {
		authenticated: false,
		name: '',
		message: ''
	});
...
{#if appState.ready}
	<section>
		...
		<button type="button" onclick={syncMessage}>Sync message</button>
	</section>
{:else}
	<p>Loading...</p>
{/if}
